...
export default class AppleGameScene extends Phaser.Scene {
	...
	private _appleGridConfig!: { baseX: number; spacingX: number; gridCols: number; gridRows: number; spacingY: number; baseY: number };
	...
	private calculateGridConfig(gridCols: number, gridRows: number): void {
		...
		const availableWidth = canvasWidth - timerBarWidth - timerBarMarginRight - 2 * margin - extraLeftMargin;
		...
		const appleSize = Math.min(appleWidthSpace, appleHeightSpace, baseAppleSize);

		// 간격 계산 (가로는 좁게, 세로는 넓게)
		const spacingX = gridCols > 1 ? (availableWidth - appleSize) / (gridCols - 1) : 0;
		const spacingY = gridRows > 1 ? (availableHeight - appleSize) / (gridRows - 1) : 0;

		// 그리드 전체 너비 계산
		const totalGridWidth = (gridCols - 1) * spacingX + appleSize;

		// 왼쪽 정렬 (추가 여백 포함)
		const baseX = margin + extraLeftMargin + (availableWidth - totalGridWidth) / 2;
	...
	create() {
		...
		this.events.on('updatePlayers', (data: { playerCount: number; players: { id: string; name: string; score: number; color: string }[]; currentPlayerIndex: number; preset?: AppleGamePreset }) => {
			...
			if (!this.isGameInitialized) {
				...
				if (data.preset) {
					...
					// 그리드 크기에 맞춰 레이아웃 재계산
					this.calculateGridConfig(resolvedConfig.gridCols, resolvedConfig.gridRows);
					...
					console.log('🎮 프리셋 적용 (초기화 전):', data.preset, '→', resolvedConfig);
				}
