...
export default class RankOnePlayerPrefab extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x?: number, y?: number, ratio: number = 1) {
    ...
    /* START-USER-CTR-CODE */
    // PlayerRank
    this.playerRank = scene.add.rectangle(
      3 * ratio,
      this.BASE_PLAYER_RANK_Y,
      210 * ratio,
      this.BASE_HEIGHT,
    );
    ...
    // playerNameText
    this.playerNameText = scene.add.text(
      4 * ratio,
      this.BASE_PLAYER_NAME_TEXT_Y,
      '',
      {},
    );
  ...
  setRank(rank: number): this {
    ...
    const rankConfig: Record<
      number,
      { height: number; crownTint?: number; crownVisible: boolean }
    > = {
