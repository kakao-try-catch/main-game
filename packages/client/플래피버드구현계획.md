# 협동 플래피버드 구현 계획

## 📋 개요

Phaser 3와 TypeScript를 사용하여 **협동 멀티플레이어 플래피버드**를 구현합니다.

### 핵심 컨셉

> [!IMPORTANT]
> **협동 게임플레이**: 각 플레이어가 자신의 새를 조종하지만, 모든 새가 밧줄로 연결되어 서로 영향을 받습니다. **한 마리라도 충돌하면 전체 게임 오버**입니다.

**새 배치**: P4 ← P3 ← P2 ← P1 (가로 일직선, 밧줄로 연결)

---

## 🎮 게임 메커니즘

### 1. 새(Bird) 시스템

**배치**:
- 4마리가 가로로 일직선 배치
- 간격: 약 80-100px
- 순서: 맨 앞(P1) → 맨 뒤(P4)

**물리 연결**:
```
P4 ←[밧줄]← P3 ←[밧줄]← P2 ←[밧줄]← P1
```

**상호작용**:
- 각 플레이어는 자신의 새만 점프 가능
- 밧줄로 연결되어 있어 다른 새의 움직임에 영향받음
- 밧줄 장력: 거리가 멀어지면 당기는 힘 발생
- 밧줄 탄성: 스프링처럼 부드럽게 연결

### 2. 밧줄(Rope) 시스템

**Phaser 구현 방법**:
- **Distance Constraint** 사용 (Phaser.Physics.Arcade에는 없음)
- 대안: **수동 구현** - 매 프레임 거리 계산 후 힘 적용

**밧줄 로직**:
```typescript
// 두 새 사이의 거리 계산
const distance = Phaser.Math.Distance.Between(bird1.x, bird1.y, bird2.x, bird2.y);
const targetDistance = 80; // 기본 밧줄 길이

// 거리가 멀어지면 당기는 힘
if (distance > targetDistance) {
    const force = (distance - targetDistance) * 0.5; // 장력 계수
    // bird1을 bird2 방향으로, bird2를 bird1 방향으로 당김
}
```

**시각적 표현**:
- 새들 사이에 선(Line) 그리기
- 색상: 플레이어 색상 또는 흰색
- 두께: 2-3px

### 3. 입력 시스템

**각 플레이어별 키 바인딩**:
```typescript
P1: Space 또는 클릭
P2: W 키
P3: I 키  
P4: ↑ (화살표 위)
```

**점프 메커니즘**:
- 자신의 새에만 위로 힘 적용
- 밧줄로 연결되어 있어 다른 새도 간접적으로 영향받음

### 4. 충돌 감지

**게임 오버 조건** (하나라도 해당하면 즉시 종료):
- 새 중 하나라도 파이프 충돌
- 새 중 하나라도 바닥 충돌
- 새 중 하나라도 천장 충돌

### 5. 점수 시스템

**협동 점수**:
- 모든 새가 파이프를 통과해야 +1점
- 개인 점수 없음, 팀 점수만 존재
- 생존 시간 보너스 (선택사항)

---

## 🏗️ 프로젝트 구조

```
src/game/flappy/
├── FlappyGameManager.ts      # 게임 메인 로직
├── BirdPrefab.ts              # 새 프리팹
├── RopeSystem.ts              # 밧줄 물리 시스템 (NEW)
├── PipePrefab.ts              # 파이프 프리팹
└── FlappyInputManager.ts      # 입력 관리 (NEW)
```

---

## 📐 구현 상세

### 1. BirdPrefab 수정

```typescript
export default class BirdPrefab extends Phaser.GameObjects.Container {
    private body: Phaser.Physics.Arcade.Body;
    private sprite: Phaser.GameObjects.Rectangle;
    private playerIndex: number;
    private playerColor: number;
    
    // 밧줄 연결용
    public connectedBirds: BirdPrefab[] = [];
    
    constructor(scene: Phaser.Scene, x: number, y: number, playerIndex: number, color: number) {
        super(scene, x, y);
        
        this.playerIndex = playerIndex;
        this.playerColor = color;
        
        // 새 스프라이트 (플레이어 색상)
        this.sprite = scene.add.rectangle(0, 0, 40, 30, color);
        this.add(this.sprite);
        
        // 물리 활성화
        scene.physics.add.existing(this);
        this.body = this.body as Phaser.Physics.Arcade.Body;
        this.body.setGravityY(600); // 중력
        this.body.setCollideWorldBounds(false); // 천장/바닥 충돌은 수동 감지
    }
    
    jump(): void {
        this.body.setVelocityY(-300);
    }
    
    update(): void {
        // 회전 각도 조절
        const angle = Phaser.Math.Clamp(this.body.velocity.y * 0.1, -20, 60);
        this.setAngle(angle);
    }
    
    // 밧줄 연결 추가
    connectTo(bird: BirdPrefab): void {
        this.connectedBirds.push(bird);
    }
}
```

### 2. RopeSystem 구현 (NEW)

```typescript
export default class RopeSystem {
    private scene: Phaser.Scene;
    private birds: BirdPrefab[];
    private ropeLines: Phaser.GameObjects.Line[] = [];
    
    // 밧줄 설정
    private readonly ROPE_LENGTH = 80; // 기본 밧줄 길이
    private readonly ROPE_STIFFNESS = 0.3; // 장력 계수 (0~1)
    private readonly ROPE_DAMPING = 0.95; // 감쇠 계수
    
    constructor(scene: Phaser.Scene, birds: BirdPrefab[]) {
        this.scene = scene;
        this.birds = birds;
        
        // 밧줄 시각화 라인 생성
        for (let i = 0; i < birds.length - 1; i++) {
            const line = scene.add.line(0, 0, 0, 0, 0, 0, 0xFFFFFF, 0.8);
            line.setLineWidth(3);
            this.ropeLines.push(line);
        }
    }
    
    update(): void {
        // 인접한 새들 사이의 밧줄 물리 적용
        for (let i = 0; i < this.birds.length - 1; i++) {
            const bird1 = this.birds[i];
            const bird2 = this.birds[i + 1];
            
            this.applyRopeConstraint(bird1, bird2);
            this.updateRopeLine(i, bird1, bird2);
        }
    }
    
    private applyRopeConstraint(bird1: BirdPrefab, bird2: BirdPrefab): void {
        const dx = bird2.x - bird1.x;
        const dy = bird2.y - bird1.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        
        // 거리 차이 계산
        const diff = distance - this.ROPE_LENGTH;
        
        if (Math.abs(diff) > 1) { // 1px 이상 차이날 때만 적용
            // 정규화된 방향 벡터
            const nx = dx / distance;
            const ny = dy / distance;
            
            // 장력 계산
            const force = diff * this.ROPE_STIFFNESS;
            
            // 양쪽 새에 반대 방향 힘 적용
            const body1 = bird1.body as Phaser.Physics.Arcade.Body;
            const body2 = bird2.body as Phaser.Physics.Arcade.Body;
            
            body1.velocity.x += nx * force;
            body1.velocity.y += ny * force;
            body2.velocity.x -= nx * force;
            body2.velocity.y -= ny * force;
            
            // 감쇠 적용 (진동 방지)
            body1.velocity.x *= this.ROPE_DAMPING;
            body1.velocity.y *= this.ROPE_DAMPING;
            body2.velocity.x *= this.ROPE_DAMPING;
            body2.velocity.y *= this.ROPE_DAMPING;
        }
    }
    
    private updateRopeLine(index: number, bird1: BirdPrefab, bird2: BirdPrefab): void {
        const line = this.ropeLines[index];
        line.setTo(bird1.x, bird1.y, bird2.x, bird2.y);
    }
    
    destroy(): void {
        this.ropeLines.forEach(line => line.destroy());
    }
}
```

### 3. FlappyInputManager 구현 (NEW)

```typescript
export default class FlappyInputManager {
    private scene: Phaser.Scene;
    private birds: BirdPrefab[];
    private keys: { [key: string]: Phaser.Input.Keyboard.Key } = {};
    
    constructor(scene: Phaser.Scene, birds: BirdPrefab[]) {
        this.scene = scene;
        this.birds = birds;
        
        this.setupKeys();
    }
    
    private setupKeys(): void {
        const keyBindings = [
            { key: 'SPACE', playerIndex: 0 }, // P1
            { key: 'W', playerIndex: 1 },     // P2
            { key: 'I', playerIndex: 2 },     // P3
            { key: 'UP', playerIndex: 3 },    // P4
        ];
        
        keyBindings.forEach(({ key, playerIndex }) => {
            if (playerIndex < this.birds.length) {
                const keyObj = this.scene.input.keyboard?.addKey(key);
                if (keyObj) {
                    keyObj.on('down', () => {
                        this.birds[playerIndex].jump();
                    });
                }
            }
        });
        
        // 마우스 클릭 (P1만)
        this.scene.input.on('pointerdown', () => {
            if (this.birds.length > 0) {
                this.birds[0].jump();
            }
        });
    }
}
```

### 4. FlappyGameManager 수정

```typescript
export default class FlappyGameManager {
    private scene: Phaser.Scene;
    private birds: BirdPrefab[] = [];
    private ropeSystem!: RopeSystem;
    private inputManager!: FlappyInputManager;
    private pipes: PipePrefab[] = [];
    private score: number = 0; // 팀 점수
    private gameOver: boolean = false;
    
    init(playerCount: number, playerColors: number[]): void {
        // 새들을 가로로 배치 (P1이 맨 앞)
        const startX = 150;
        const startY = 300;
        const spacing = 80;
        
        for (let i = 0; i < playerCount; i++) {
            const bird = new BirdPrefab(
                this.scene,
                startX + i * spacing,
                startY,
                i,
                playerColors[i]
            );
            this.birds.push(bird);
            this.scene.add.existing(bird);
        }
        
        // 밧줄 시스템 초기화
        this.ropeSystem = new RopeSystem(this.scene, this.birds);
        
        // 입력 관리자 초기화
        this.inputManager = new FlappyInputManager(this.scene, this.birds);
        
        // 파이프 생성 시작
        this.startPipeSpawner();
    }
    
    update(): void {
        if (this.gameOver) return;
        
        // 새 업데이트
        this.birds.forEach(bird => bird.update());
        
        // 밧줄 물리 적용
        this.ropeSystem.update();
        
        // 충돌 감지 (한 마리라도 충돌하면 게임 오버)
        this.checkCollisions();
        
        // 파이프 스크롤
        this.updatePipes();
    }
    
    private checkCollisions(): void {
        for (const bird of this.birds) {
            // 천장/바닥 충돌
            if (bird.y < 0 || bird.y > this.scene.scale.height) {
                this.onGameOver();
                return;
            }
            
            // 파이프 충돌
            for (const pipe of this.pipes) {
                if (this.checkBirdPipeCollision(bird, pipe)) {
                    this.onGameOver();
                    return;
                }
            }
        }
    }
    
    private onGameOver(): void {
        this.gameOver = true;
        console.log('🎮 게임 오버! 최종 점수:', this.score);
        this.scene.events.emit('gameEnd', { score: this.score });
    }
}
```

---

## 🎨 UI/UX 개선

### 플레이어 표시

**화면 상단에 플레이어 정보 표시**:
```
P1 [Space] | P2 [W] | P3 [I] | P4 [↑]
```

**각 플레이어 색상**:
- P1: 파란색 (#209cee)
- P2: 빨간색 (#e76e55)
- P3: 초록색 (#92cc41)
- P4: 노란색 (#f2d024)

### 점수 표시

**중앙 상단**:
```
협동 점수: 15
```

---

## ✅ 검증 계획

### 1. 밧줄 물리 테스트

1. 새 1마리만 점프 → 다른 새들도 약간 끌려올라가는지 확인
2. 새들 간격이 멀어지면 → 당기는 힘이 작용하는지 확인
3. 새들이 너무 가까워지면 → 밀어내는지 또는 자연스럽게 유지되는지 확인

### 2. 협동 게임플레이 테스트

1. 2명 플레이: P1만 점프, P2는 가만히 → 협력 필요성 확인
2. 4명 플레이: 모두 다른 타이밍에 점프 → 혼란스러운지 확인
3. 한 명이 파이프에 충돌 → 전체 게임 오버 확인

### 3. 난이도 밸런싱

- 밧줄 장력 조절 (ROPE_STIFFNESS)
- 밧줄 길이 조절 (ROPE_LENGTH)
- 파이프 간격 조절

---

## 📝 구현 단계

### Phase 1: 기본 새 + 밧줄 시스템 (2시간)
- [ ] `BirdPrefab` 구현
- [ ] `RopeSystem` 구현
- [ ] 밧줄 물리 테스트

### Phase 2: 입력 + 파이프 (1.5시간)
- [ ] `FlappyInputManager` 구현
- [ ] `PipePrefab` 구현
- [ ] 파이프 스크롤 및 생성

### Phase 3: 충돌 + 점수 (1시간)
- [ ] 충돌 감지 (한 마리라도 충돌 시 게임 오버)
- [ ] 협동 점수 시스템
- [ ] 게임 오버 처리

### Phase 4: 로비 통합 + 폴리싱 (1시간)
- [ ] 로비에서 플레이어 수 설정
- [ ] UI 개선 (플레이어 키 안내)
- [ ] 밸런싱

**총 예상 시간**: 5-6시간

---

## 🎯 성공 기준

- ✅ 새들이 밧줄로 연결되어 서로 영향을 받음
- ✅ 각 플레이어가 자신의 새만 조종 가능
- ✅ 한 마리라도 충돌하면 게임 오버
- ✅ 협동 점수 시스템 작동
- ✅ 1-4명 플레이어 지원
- ✅ 밧줄이 시각적으로 표시됨

---

## 💡 밸런싱 팁

**밧줄 설정 권장값**:
- `ROPE_LENGTH`: 80-100px (플레이어 수에 따라 조절)
- `ROPE_STIFFNESS`: 0.2-0.4 (너무 높으면 딱딱함)
- `ROPE_DAMPING`: 0.9-0.98 (진동 방지)

**난이도별 조절**:
- Easy: 밧줄 길이 ↑, 장력 ↓, 파이프 간격 ↑
- Normal: 기본값
- Hard: 밧줄 길이 ↓, 장력 ↑, 파이프 간격 ↓
